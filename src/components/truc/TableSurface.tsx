    ...
    <div className="relative w-full h-full min-h-[420px] pointer-events-none">
      ...
      <div className="absolute inset-5 rounded-[46%/38%] felt-surface border border-primary/20 overflow-hidden pointer-events-none">
        ...
        {!hideAllPlayedCards && ([0, 1, 2, 3] as PlayerId[]).map((p) => {
          ...
          // La pila del jugador que té la ÚLTIMA carta jugada (la del
          // reborde groc / animació `animate-play`) ha de flotar per
          // damunt de les piles dels altres jugadors. Z-index dinàmic
          // al contenidor pare perquè la carta i el seu reborde no
          // queden tapats per cartes ja presents a la mesa.
          const isLatestPlayer = !!currentTC && currentTC.card.id === latestPlayedCardId;
          return (
            <div
              key={`pile-${p}`}
              className={cn(
