...
import { getRoom, getMyHand, heartbeat } from "./rooms.functions";
...
import type { Action, Card, MatchState, PlayerId } from "@/game/types";
import { applyAction } from "@/game/engine";
import { VISUAL_EVENT_GAP_MS } from "@/game/chatTimings";

function isHiddenCard(c: unknown): boolean {
  if (!c || typeof c !== "object") return false;
  const obj = c as { hidden?: boolean; id?: string };
  return obj.hidden === true || (typeof obj.id === "string" && obj.id.startsWith("hidden-"));
}

/**
 * Si el `row` entrante trae la mano del jugador (mySeat) enmascarada
 * (placeholders `hidden-*`), intenta inyectar las cartas REALES desde el
 * snapshot previo si encajan en longitud. Si no se puede (longitud distinta
 * = mano nueva tras reparto), devuelve `needsFetch=true` para que el caller
 * llame a `getMyHand`. ESTO ES LA CAPA DE PRIVACIDAD CLIENTE: nunca
 * recibimos cartas de rivales por broadcast; sólo pedimos las nuestras.
 */
function injectMyHandIntoRow(
  row: RoomRowPayload,
  mySeat: PlayerId | null,
  prev: MatchState | null,
): { row: RoomRowPayload; needsFetch: boolean } {
  if (mySeat == null || !row.match_state) return { row, needsFetch: false };
  const incomingHand = (row.match_state.round?.hands?.[mySeat] ?? []) as Card[];
  const incomingHidden =
    incomingHand.length > 0 && incomingHand.every((c) => isHiddenCard(c));
  if (!incomingHidden) return { row, needsFetch: false };
  const prevHand = (prev?.round?.hands?.[mySeat] ?? []) as Card[];
  const prevHasReal =
    prevHand.length > 0 && prevHand.every((c) => !isHiddenCard(c));
  if (prevHasReal && prevHand.length === incomingHand.length) {
    const patched: RoomRowPayload = {
      ...row,
      match_state: {
        ...row.match_state,
        round: {
          ...row.match_state.round,
          hands: { ...row.match_state.round.hands, [mySeat]: prevHand },
        },
      } as MatchState,
    };
    return { row: patched, needsFetch: false };
  }
  return { row, needsFetch: true };
}
...
export function useRoomRealtime(code: string | null, deviceId: string, perspectiveOverride: PlayerId | null = null) {
  const [data, setData] = useState<RoomFullDTO | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const dataRef = useRef<RoomFullDTO | null>(null);
  const perspectiveOverrideRef = useRef<PlayerId | null>(perspectiveOverride);
  perspectiveOverrideRef.current = perspectiveOverride;
  ...
  /** Guard against firing parallel `getMyHand` requests. */
  const fetchingMyHandRef = useRef(false);
  dataRef.current = data;

  /**
   * Llama a la RPC `getMyHand` para refrescar la mano REAL del jugador
   * desde el servidor (sólo el propio asiento; nunca rivales). Se invoca
   * cuando llega un row enmascarado que no podemos reconstruir desde el
   * snapshot previo (típicamente: nuevo reparto al inicio de ronda).
   */
  const fetchMyHand = useCallback(async () => {
    if (!code || !deviceId) return;
    if (fetchingMyHandRef.current) return;
    fetchingMyHandRef.current = true;
    try {
      const { hand, seat } = await getMyHand({ data: { code, deviceId } });
      const cur = dataRef.current;
      if (!cur || !cur.room.matchState || seat == null) return;
      const ms = cur.room.matchState;
      const next: MatchState = {
        ...ms,
        round: {
          ...ms.round,
          hands: { ...ms.round.hands, [seat]: hand as Card[] },
        },
      };
      setData({ ...cur, room: { ...cur.room, matchState: next } });
    } catch {
      /* ignore — siguiente row volverá a disparar la petición */
    } finally {
      fetchingMyHandRef.current = false;
    }
  }, [code, deviceId]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const override = perspectiveOverrideRef.current;
      if (override != null && dto.room.matchState) {
        const remasked = maskMatchStateForSeat(dto.room.matchState, override, null);
        setData({ ...dto, room: { ...dto.room, matchState: remasked } });
      } else {
        setData(dto);
      }
      setError(null);
  ...
  const applyRoomRow = useCallback((row: RoomRowPayload) => {
    ...
    // Inyecta la mano REAL del propio asiento (el row sólo trae placeholders
    // para todos). Si no podemos reconstruirla (nuevo reparto), disparamos
    // `getMyHand` para pedirla al servidor.
    const mySeatNow = perspectiveOverrideRef.current ?? prev.mySeat;
    const injected = injectMyHandIntoRow(row, mySeatNow, prevState);
    row = injected.row;
    if (injected.needsFetch) {
      // No bloqueamos el render: aplicamos el row con la mano enmascarada
      // y la sobreescribimos en cuanto `getMyHand` responda.
      void fetchMyHand();
    }
    ...
    const nextState = row.match_state
      ? maskMatchStateForSeat(row.match_state, perspectiveOverrideRef.current ?? prev.mySeat, prevState)
      : null;
  ...
  const applyOptimistic = useCallback((player: PlayerId, action: Action): {
    ...
    const masked = maskMatchStateForSeat(nextMatch, perspectiveOverrideRef.current ?? prev.mySeat, prevMatchState);
    ...
    return {
      ...
      rollback: () => {
        ...
        if (curLogLen >= nextMatch.round.log.length) {
          const remasked = maskMatchStateForSeat(prevMatchState, perspectiveOverrideRef.current ?? cur.mySeat, cur.room.matchState);
          setData({
            ...cur,
  ...
  useEffect(() => {
    ...
    const onWake = () => {
      if (cancelled) return;
      // Reconnect first; the SUBSCRIBED callback performs the authoritative
      // refresh only after the transport is stable again.
      teardown();
      attempts = 0;
      clearReconnectTimer();
      connect();
