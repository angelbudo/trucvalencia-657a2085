...
import {
  ...
  randomSecondPlayerWaitMs,
  OPENER_WAIT_FOR_PARTNER_INFO_MS,
  FIRST_TRICK_OPENER_MIN_PLAY_MS,
...
export function useTrucMatch(options: UseTrucMatchOptions = {}) {
  ...
  // Marca temporal de l'últim chat emès per cada jugador. S'usa per
  // afegir una pausa "dramàtica" d'1 segon al bot quan li toca el torn
  // just després que el seu company haja parlat.
  const lastChatAtByPlayerRef = useRef<Map<PlayerId, number>>(new Map());
  const recordChatPhrase = useCallback((player: PlayerId, phraseId: ChatPhraseId) => {
    ...
    lastChatAtByPlayerRef.current.set(player, Date.now());
    if (
  ...
  // Marca temporal de l'última vegada que un jugador ha rebut una
  // pregunta de chat (categoria "pregunta"). S'usa per a cancel·lar
  // missatges espontanis d'iniciativa pròpia: si el company li acaba
  // de preguntar, el bot ha de respondre en compte de soltar una
  // frase aleatòria.
  const QUESTION_PHRASE_IDS: ReadonlySet<ChatPhraseId> = new Set<ChatPhraseId>([
    "puc-anar", "que-tens", "tens-mes-dun-tres", "portes-un-tres",
    "tens-envit", "vols-envide", "vols-tornar-envidar", "quant-envit",
  ]);
  const questionReceivedAtRef = useRef<Map<PlayerId, number>>(new Map());
  ...
  sayRef.current = (player, phraseId, durationMs, vars) => {
    ...
    if (QUESTION_PHRASE_IDS.has(phraseId)) {
      questionReceivedAtRef.current.set(partnerOf(player), Date.now());
    }
  ...
  // ---------------------------------------------------------------------------
  // Regla universal de temps entre cartes:
  //  · 1000 ms entre qualsevol carta tirada i l'acció següent.
  //  · 2000 ms quan la carta acabada de tirar tanca la baza (3a/última carta),
  //    perquè es vega clarament qui guanya abans de continuar.
  // El bloqueig s'aplica a humà i bots a través de `dispatch` i del scheduler
  // de bots (afegim el temps restant al `delay` calculat).
  // ---------------------------------------------------------------------------
  const cardFreezeUntilRef = useRef<number>(0);
  const [cardFreezeTick, setCardFreezeTick] = useState(0);
  const remainingCardFreezeMs = () => Math.max(0, cardFreezeUntilRef.current - Date.now());
  ...
  // REHIDRATACIÓ DE TORN: en muntar el hook (incloent restauració d'una
  // partida guardada o reentrada online) i cada vegada que es desactiva
  // la pausa, neteja qualsevol timer/consulta pendent d'una vida anterior
  // i força una reavaluació del scheduler. Si el torn actual pertany a un
  // bot, el useEffect principal (deps [match, paused, mandatorySecondWaitTick])
  // es tornarà a executar i el programarà; el bump del tick garanteix que
  // ho fa encara que `match` i `paused` no hagen canviat de referència
  // (cas típic en mount inicial després d'un resume).
  useEffect(() => {
    // Neteja preventiva de timers vells (sense entrar en pausa).
    if (timerRef.current) {
      window.clearTimeout(timerRef.current);
      timerRef.current = null;
    }
    ...
    if (options.paused) return;
    // Força una reavaluació del scheduler perquè detecte si toca a un bot
    // i programe la seua acció (treu el bot del bucle congelat).
  ...
  // En el primer render hem de SEMBRAR la longitud actual de l'historial
  // (i NO armar el grace) — altrament, en restaurar una partida guardada
  // (resume=true) on history.length > 0, el flag s'armaria i mai es
  // desactivaria (el pare no programarà cap animació de transició perquè
  // ningú no acaba una mà), congelant la partida.
  const historySeedRef = useRef<boolean>(false);
  if (!historySeedRef.current) {
    lastHistoryLenRef.current = match.history.length;
    historySeedRef.current = true;
  } else if (lastHistoryLenRef.current !== match.history.length) {
  ...
  const rejectedShoutByPlayer = display.rejectedShoutByPlayer;
  ...
  const completedMandatorySecondWaitsRef = useRef<Set<string>>(new Set());
  const [mandatorySecondWaitTick, setMandatorySecondWaitTick] = useState(0);
  const lastEnvitResolutionCleanupKeyRef = useRef<string | null>(null);
  /**
  ...
  /**
   * Missatge espontani pendent (encara dins de la finestra de 3s) per a
   * cada bot. Si el seu company li fa una pregunta abans que el
   * temporitzador dispari, el cancel·lem i guardem la frase a
   * `capturedSpontaneousRef` per fer el filtre de duplicats en la resposta.
   */
  const pendingSpontaneousRef = useRef<
    Partial<Record<PlayerId, { phrase: ChatPhraseId; timer: number }>>
  >({});
  const capturedSpontaneousRef = useRef<
    Partial<Record<PlayerId, ChatPhraseId>>
  >({});
  const cancelPendingSpontaneous = (player: PlayerId): void => {
    const entry = pendingSpontaneousRef.current[player];
    if (!entry) return;
    window.clearTimeout(entry.timer);
    capturedSpontaneousRef.current[player] = entry.phrase;
    delete pendingSpontaneousRef.current[player];
  };
  const respondWithDedup = (
    speaker: PlayerId,
    answer: ChatPhraseId,
    vars?: Record<string, string | number>,
    bubble?: number,
  ): void => {
    cancelPendingSpontaneous(speaker);
    const captured = capturedSpontaneousRef.current[speaker];
    delete capturedSpontaneousRef.current[speaker];
    sayRef.current?.(speaker, answer, bubble, vars);
    if (captured && captured !== answer) {
      window.setTimeout(() => {
        sayRef.current?.(speaker, captured);
      }, 1500);
    }
  };
  /**
  ...
  const proactiveEnvitInstructionRef = useRef<Set<string>>(new Set());
  /**
  ...
  const isBotPlayBlockedByPartnerConsult = useCallback((player: PlayerId): boolean => {
    if (pendingHumanAnswerRef.current?.botPlayer === player) return true;
    if (pendingSecondWaitRef.current?.botPlayer === player) return true;
    if (pendingOpenerWaitRef.current?.botPlayer === player) return true;
    for (const consultKey of consultInFlightRef.current) {
      if (consultKey.endsWith(`-${player}`)) return true;
    }
    return false;
  }, []);
    ...
    if (waiting && player === partnerOf(waiting.botPlayer)) {
      ...
      if (accepted.includes(phraseId)) {
        if (waiting.mandatory) {
          ...
          // (o que tire la falta) durant la finestra obligatòria de
          ...
          if (phraseId === "envida" || phraseId === "tira-falta") {
            ...
            if (waiting.partnerBotTimer) {
              window.clearTimeout(waiting.partnerBotTimer);
            }
            pendingSecondWaitRef.current = null;
  ...
  // Timers de chats d'iniciativa pròpia de bots (categoria "indicacio").
  // Cada timer s'identifica per `chat-iniciativa-${botId}` per a poder-lo
  // cancel·lar quan el seu company tira carta abans que arribi a pintar-se.
  const initiativeChatTimers = usePausableTimers(false);
  ...
  const dispatch = useCallback((player: PlayerId, action: Action) => {
    ...
    // Cancel·lació per acció realitzada: si es tira una carta a la mesa,
    // fulminem qualsevol xat d'iniciativa pròpia pendent del company del
    // jugador que acaba de tirar (control de desfase).
    if (action.type === "play-card") {
      initiativeChatTimers.clearByKey(`chat-iniciativa-${partnerOf(player)}`);
      // Arma el freeze entre cartes (1 s normal / 2 s en fi de baza).
      const prevForFreeze = matchRef.current;
      if (prevForFreeze) {
        try {
          const candidate = applyAction(prevForFreeze, player, action);
          const newEvents = candidate.round.log.slice(prevForFreeze.round.log.length);
          const trickEnded = newEvents.some((e) => e.type === "trick-end");
          const freezeMs = trickEnded ? 2000 : 1000;
          cardFreezeUntilRef.current = Date.now() + freezeMs;
          window.setTimeout(() => setCardFreezeTick((t) => t + 1), freezeMs + 16);
        } catch {
          /* acció il·legal: ignorem i no armem freeze */
        }
      }
    }
    ...
    // Si està actiu el cartell central de "Vull!"/"No vull", no descartem
    // la carta del bot: la reprogramem. Abans es feia `return` directe i,
    // si el timer havia disparat uns ms massa prompte després d'un "No vull",
    // el match no canviava i l'efecte no tornava a programar el bot.
    if (
      player !== HUMAN &&
      ...
      timerRef.current = window.setTimeout(() => {
        ...
        const stillLegal = legalActions(current, player).some((a) =>
          a.type === "play-card" && a.cardId === action.cardId,
        );
        if (!stillLegal) return;
        setMatch((prev) => applyAction(prev, player, action));
  ...
  }, [enqueueLocalShoutFlash, isBotPlayBlockedByPartnerConsult, isFirstOfPair]);
  ...
  const newRound = useCallback(() => {
    ...
    proactiveEnvitInstructionRef.current.clear();
    completedMandatorySecondWaitsRef.current.clear();
  ...
  useEffect(() => {
    ...
    // Si el peu-bot estava esperant a la finestra "vols-envide" (2n de la
    // 1a baza, envit encara no cantat) i mentrestant algú ha cantat envit
    // (o l'envit ja s'ha resolt), aquella espera ja no té sentit: cal
    // netejar-la o l'efecte es bloqueja indefinidament a l'early-return
    // de pendingSecondWaitRef quan el torn torna al bot després del
    // "No vull".
    {
      if (r.envitResolved) {
        const cleanupKey = `${match.history.length}-${r.log.length}`;
        if (lastEnvitResolutionCleanupKeyRef.current !== cleanupKey) {
          lastEnvitResolutionCleanupKeyRef.current = cleanupKey;
          clearConsultTimers();
          const pendingHuman = pendingHumanAnswerRef.current;
          if (pendingHuman) {
            window.clearTimeout(pendingHuman.timer);
            pendingHumanAnswerRef.current = null;
          }
        }
      }
      const _pw = pendingSecondWaitRef.current;
      if (_pw && (r.envitState.kind !== "none" || r.envitResolved)) {
        window.clearTimeout(_pw.timer);
        if (_pw.partnerBotTimer) window.clearTimeout(_pw.partnerBotTimer);
        pendingSecondWaitRef.current = null;
        consultStartedRef.current.delete(_pw.waitKey);
      }
      // Mateixa idea per a la finestra silenciosa de l'opener (Feature A):
      // si l'envit / truc s'ha mogut o ja s'ha resolt durant l'espera,
      // cancel·la-la per evitar bloquejos quan el torn torna al bot.
      const _po = pendingOpenerWaitRef.current;
      if (_po && (r.envitState.kind !== "none" || r.envitResolved || r.trucState.kind === "pending" || r.tricks.length !== 1)) {
        window.clearTimeout(_po.timer);
        pendingOpenerWaitRef.current = null;
        consultInFlightRef.current.delete(_po.consultKey);
        consultStartedRef.current.delete(_po.consultKey);
      }
      // Si una consulta humana específica d'envit quedó viva mientras el
      // envit se resolvió con "No vull", no puede bloquear el turno normal
      // del mismo bot al volverle la mano en la primera baza.
      const _ph = pendingHumanAnswerRef.current;
      if (_ph && (_ph.consultKey.startsWith("envit-consult-") || _ph.consultKey.startsWith("truc-envit-consult-"))) {
        const botTeam = teamOf(_ph.botPlayer);
        const stillAwaitingEnvit = r.envitState.kind === "pending" && r.envitState.awaitingTeam === botTeam;
        const stillAwaitingTruc = r.trucState.kind === "pending" && r.trucState.awaitingTeam === botTeam;
        if (!stillAwaitingEnvit && !stillAwaitingTruc) {
          window.clearTimeout(_ph.timer);
          pendingHumanAnswerRef.current = null;
          consultInFlightRef.current.delete(_ph.consultKey);
          consultStartedRef.current.delete(_ph.consultKey);
        }
      }
    }
    ...
    // Bloqueig prioritari: en la primera baza, el segon jugador de cada
    // parella no pot iniciar cap consulta ni decisió automàtica fins que
    // haja transcorregut una espera aleatòria completa de 7–10 segons.
    // Qualsevol envit ja cantat anul·la immediatament aquest bloqueig.
    const mandatoryFirstTrick = r.tricks[0];
    const mandatoryPartner = partnerOf(botPlayer);
    // Excepció (iniciativa directa): si el propi bot té envit (≥30) i pot
    // cantar envit en aquest moment, no espera la finestra obligatòria de
    // 7-10 s ni consulta al company: envida directament. Així el joc no
    // s'alenteix quan el bot ja té punts clars per envidar pel seu compte.
    const mandatoryBotCanEnvitNow = legalActions(match, botPlayer).some(
      (a) => a.type === "shout" && a.what === "envit",
    );
    const mandatoryBotHasHighEnvit =
      mandatoryBotCanEnvitNow && playerTotalEnvit(r, botPlayer) >= 30;
    const mandatoryWaitApplies =
      r.tricks.length === 1 &&
      r.turn === botPlayer &&
      r.envitState.kind === "none" &&
      !r.envitResolved &&
      mandatoryFirstTrick.cards.some((tc) => tc.player === mandatoryPartner) &&
      !mandatoryFirstTrick.cards.some((tc) => tc.player === botPlayer) &&
      !mandatoryBotHasHighEnvit;
    const mandatoryWaitKey = `mandatory-wait2-${match.history.length}-${r.mano}-${botPlayer}-${mandatoryFirstTrick.cards.length}`;
    if (
      mandatoryWaitApplies &&
      !completedMandatorySecondWaitsRef.current.has(mandatoryWaitKey)
    ) {
      if (pendingSecondWaitRef.current?.waitKey !== mandatoryWaitKey) {
        clearPendingSecondWait();
        const waitMs = randomSecondPlayerWaitMs();
        const timer = window.setTimeout(() => {
          if (pendingSecondWaitRef.current?.waitKey !== mandatoryWaitKey) return;
          pendingSecondWaitRef.current = null;
          completedMandatorySecondWaitsRef.current.add(mandatoryWaitKey);
          setMandatorySecondWaitTick((tick) => tick + 1);
        }, waitMs) as unknown as number;
        pendingSecondWaitRef.current = {
          botPlayer,
          waitKey: mandatoryWaitKey,
          timer,
          partnerBotTimer: null,
          mandatory: true,
          resolve: () => {},
        };
      }
    ...
    // Regla universal entre cartes: 1 s normal / 2 s en fi de baza.
    delay = Math.max(delay, remainingCardFreezeMs());

    // Pausa "dramàtica" de reacció: si el company del bot ha emès un
    // chat (frase, pregunta, senyal) en l'últim segon, el bot espera
    // com a mínim 1000 ms abans d'executar la seua acció, per simular
    // que està reaccionant al missatge en compte de respondre instantani.
    {
      const partnerChatAt = lastChatAtByPlayerRef.current.get(partnerOf(botPlayer)) ?? 0;
      const sincePartnerChat = Date.now() - partnerChatAt;
      if (partnerChatAt > 0 && sincePartnerChat < 1000) {
        delay = Math.max(delay, 1000 - sincePartnerChat);
      }
    }


    // Pausa entre bazas: si el bot ha d'obrir una nova baza (2a o 3a) i
    // encara no s'ha tirat cap carta en ella, espera com a mínim 2 s
    // perquè el jugador puga veure el desenllaç de la baza anterior.
    if (
      r.turn === botPlayer &&
      r.phase === "playing" &&
      r.tricks.length >= 2 &&
    ...
    // Regla primordial: si el bot és el PRIMER de la seua parella en
    // tirar a la 1a baza i hi ha qualsevol comunicació amb el company
    // (pregunta del bot, frase espontània del company o resposta a una
    // consulta), ha d'esperar com a mínim FIRST_TRICK_OPENER_MIN_PLAY_MS
    // (7,5 s) des de l'inici del seu torn abans de tirar la carta. Així
    // donem temps real perquè la parella es coordine.
    const isOpenerFirstTrickPlay =
      isPlayCardTurn &&
      r.tricks.length === 1 &&
      isBotOpeningForTeam(match, botPlayer);
    const openerTurnStartMs = Date.now();
    /** Calcula el delay real perquè la dispatch final caiga, com a
     *  mínim, FIRST_TRICK_OPENER_MIN_PLAY_MS després de l'inici del torn
     *  quan s'aplica la regla primordial; si no, retorna `baseDelay`. */
    const openerPlayDelay = (baseDelay: number): number => {
      if (!isOpenerFirstTrickPlay) return baseDelay;
      const elapsed = Date.now() - openerTurnStartMs;
      const remaining = FIRST_TRICK_OPENER_MIN_PLAY_MS - elapsed;
      return Math.max(baseDelay, remaining);
    };

    // REGLA TÀCTICA DURA (envit en parella): davant d'un envit del rival,
    // el bot que té el torn de resposta NO pot acceptar (vull) directament
    // de forma egoista. Sempre que la seua intenció siga acceptar (vull),
    // primer ha de consultar OBLIGATÒRIAMENT al company per si aquest
    // prefereix pujar (renvit / falta-envit). El flux és:
    //   1) Pregunta al company a 1500 ms del torn ("Vols tornar a envidar?"
    //      o, si ja s'ha pujat, "Quant envit tens?").
    //   2) Resposta del company a 1500 ms de la pregunta.
    //   3) Espera 1500 ms abans de llançar la resposta final al joc.
    // Si el company té envit alt (≥31) i decideix pujar, la resposta serà
    // renvit / falta-envit; altrament, el bot fa "vull" segons la seua
    // pròpia mà.
    ...
    if (
      ...
      // Previsualitza què faria el bot SENSE consultar: si la seua
      // decisió natural és "vull" (o és obligatori per regla dura amb
      // ≥31), llavors aplica la consulta tàctica al company.
      const previewAction = botDecide(
        match,
        botPlayer,
        cachedAdvice,
        {},
      ...
      const previewWouldVull =
        previewAction?.type === "shout" && previewAction.what === "vull";
      const wouldAccept =
        (previewWouldVull || myEnvitNow >= 31) &&
        canRaiseOrAccept &&
        !partnerAlreadyRejected;
      const partnerEnv = partnerOf(botPlayer);
      const partnerIsBotEnv = partnerEnv !== HUMAN;
      // EXCEPCIÓ INICIATIVA DIRECTA (contra-envit): si el bot té 33, o
      // 32 sent "mano" respecte al rival que ha cantat l'envit, no cal
      // consultar al company: puja directament (renvit / falta-envit).
      let manoPriorityOverCaller = false;
      if (r.envitState.kind === "pending") {
        const callerNow = r.envitState.calledBy;
        let pp: PlayerId = r.mano;
        for (let i = 0; i < 4; i++) {
          if (pp === botPlayer) { manoPriorityOverCaller = true; break; }
          if (pp === callerNow) { manoPriorityOverCaller = false; break; }
          pp = ((pp + 1) % 4) as PlayerId;
        }
      }
      const raiseAct = acts.find(
        (a) => a.type === "shout" && (a.what === "renvit" || a.what === "falta-envit"),
      );
      const directRaise =
        !isCamaMatchPoint(match) &&
        !partnerAlreadyRejected &&
        raiseAct &&
        (myEnvitNow >= 33 || (myEnvitNow === 32 && manoPriorityOverCaller));
      if (directRaise && raiseAct) {
        consultStartedRef.current.add(envitConsultKey);
        consultAdviceRef.current.set(envitConsultKey, "neutral");
        scheduleConsultTimer(() => {
          dispatch(botPlayer, raiseAct);
        }, BOT_DELAY_MS);
        return;
      }
      ...
      if (wouldAccept && !isCamaMatchPoint(match)) {
        ...
        consultStartedRef.current.add(envitConsultKey);
        consultInFlightRef.current.add(envitConsultKey);
        /** Espera fluida (1,5 s) entre la resposta del company i l'acció
         *  final que el bot llança al joc, perquè la cadena de bocadillos
         *  no es solape amb el cartell central de "Vull!" / renvit. */
        const FINAL_RESPONSE_DELAY_MS = 1500;
        const finalizeAfter = (
          instruction: ChatPhraseId | null,
          partnerAnnouncedEnvit: number | null = null,
        ) => {
          // Marca consultat (qualsevol valor) per a no repetir.
          consultAdviceRef.current.set(envitConsultKey, "neutral");
          scheduleConsultTimer(() => {
            finishConsult(envitConsultKey);
            ...
            // Senyal que el company té envit alt (≥31) i vol pujar:
            //  - "envida" → resposta proactiva del company amb ≥31
            //    (shouldProactivelyOrderEnvit).
            //  - "si-tinc-n" amb n ≥ 31 → el company ha confessat el
            //    seu valor real i és prou alt per pujar.
            const partnerWantsRaise =
              instruction === "envida" ||
              (instruction === "si-tinc-n" &&
                partnerAnnouncedEnvit !== null &&
                partnerAnnouncedEnvit >= 31);

            // Si el company vol pujar, el bot puja per ell (renvit /
            // falta-envit) encara que la seua pròpia mà no donara per a
            // pujar. Així el bot no "pisa" la voluntat del company.
            if (partnerWantsRaise && currentRenvit) {
              dispatch(botPlayer, currentRenvit);
              return;
            }
            // Camí clàssic: amb la pròpia mà, si pot pujar i el company
            // diu "sí" (sense confessar valor concret prou alt), puja.
            if (
              (instruction === "si" ||
                (instruction === "si-tinc-n" && !partnerWantsRaise)) &&
              myCanRaiseNow &&
              currentRenvit
            ) {
              dispatch(botPlayer, currentRenvit);
        ...
        scheduleConsultTimer(() => {
          ...
          if (partnerIsBotEnv) {
            cancelPendingSpontaneous(partnerEnv);
            let answer: ChatPhraseId;
            let answerVars: Record<string, string | number> | undefined;
            let announcedEnvit: number | null = null;
            if (envitAlreadyRaised) {
              answer = "si-tinc-n";
              const partnerEnvitValue = playerTotalEnvit(r, partnerEnv);
              answerVars = { n: partnerEnvitValue };
              announcedEnvit = partnerEnvitValue;
            } else {
            ...
            scheduleConsultTimer(() => {
              respondWithDedup(partnerEnv, answer, answerVars);
              finalizeAfter(answer, announcedEnvit);
            }, CONSULT_BOT_ANSWER_DELAY_MS);
          } else {
            // Company humà: espera resposta o timeout.
            const tid = window.setTimeout(() => finalizeAfter(null, null), CONSULT_HUMAN_TIMEOUT_MS) as unknown as number;
    ...
    if (
      ...
      if (canEnvitTE && sayRef.current) {
        ...
        // Consulta al company.
        consultStartedRef.current.add(trucEnvitConsultKey);
        consultInFlightRef.current.add(trucEnvitConsultKey);
        const finalizeAfterTE = (instruction: ChatPhraseId | null) => {
          consultAdviceRef.current.set(trucEnvitConsultKey, "neutral");
          scheduleConsultTimer(() => {
            finishConsult(trucEnvitConsultKey);
        ...
        scheduleConsultTimer(() => {
          ...
          if (partnerIsBotTE) {
            cancelPendingSpontaneous(partnerSeatTE);
            ...
            const answerVars =
              answer === "si-tinc-n"
                ...
                : undefined;
            scheduleConsultTimer(() => {
              respondWithDedup(partnerSeatTE, answer, answerVars);
    ...
    // Regla d'envit proactiu entre bots: només el primer jugador de la
    // parella pot dir "Envida!" al segon jugador de la seua parella en la
    // 1a baza, abans que aquest tire. Amb ≥31 sempre; amb 30 només si el
    // company és 4t a tirar, en un 70%.
    const proactiveEnvitKey = `pro-envit-${match.history.length}-${botPlayer}`;
    const botPartnerForEnvit = partnerOf(botPlayer);
    ...
    if (
      ...
      shouldIssueProactiveEnvitOrder(playerTotalEnvit(r, botPartnerForEnvit), {
        trickIndex: trickIdx,
        actor: botPlayer,
        speaker: botPartnerForEnvit,
        mano: r.mano,
        envitAlreadyCalled: r.envitState.kind !== "none" || r.envitResolved,
      }) &&
      !proactiveEnvitInstructionRef.current.has(proactiveEnvitKey)
    ) {
      proactiveEnvitInstructionRef.current.add(proactiveEnvitKey);
      consultStartedRef.current.add(proactiveEnvitKey);
      scheduleConsultTimer(() => {
        sayRef.current?.(botPartnerForEnvit, "envida", bubbleDurationMs);
        scheduleConsultTimer(() => {
          const envit = legalActions(matchRef.current, botPlayer).find(
            (a) => a.type === "shout" && a.what === "envit",
          );
    ...
    // envidat. La finestra obligatòria prèvia dona temps perquè el company li
    ...
    // Espera la finestra obligatòria perquè el company li puga dir
    ...
    if (
      ...
      // Iniciativa directa: si el propi bot ja té envit (≥30), envida
      // immediatament sense esperar instruccions del company ni la
      // finestra de 8 s. Així el joc no s'alenteix quan el bot té punts
      // clars per envidar pel seu compte.
      ...
      const finalize = (instruction: ChatPhraseId | null) => {
        ...
        // També cancel·lem el xat d'iniciativa keyed per si encara estava
        // pendent (p. ex. finalització primerenca abans dels 500 ms).
        initiativeChatTimers.clearByKey(`chat-iniciativa-${partner}`);
      ...
      // La finestra obligatòria de 7–10 s ja s'ha consumit abans d'entrar
      // en aquest flux; sense instrucció pendent, decideix ara.
      const timeoutId = window.setTimeout(() => finalize(null), 0) as unknown as number;
      ...
      // Regla d'envit en parella (Regla #1): si el bot és el 2n de la seua
      // parella i NO va a envidar per iniciativa pròpia, espera 7 s
      // (7–10 s) i pot preguntar al company amb
      // probabilitat depenent de la seua posició global a la ronda:
      //   · 3a posició (cardsOnTable.length === 2) → 30%
      //   · 4a posició (cardsOnTable.length === 3) → 80%
      // Excepció dura: si és 4t i no pot guanyar la baza, sempre pregunta
      // directament (askDirectlyNoWait=true).
      const isThirdToPlay = cardsOnTable.length === 2;
      const askProbByPosition = isFourthToPlay ? 0.8 : isThirdToPlay ? 0.3 : 0.35;
      let partnerBotTimer: number | null = null;
      const mandatoryWaitCompleted = completedMandatorySecondWaitsRef.current.has(mandatoryWaitKey);
      const askPartner =
        !mandatoryWaitCompleted &&
        (askDirectlyNoWait || (canEnvit && Math.random() < askProbByPosition));
      if (askPartner && sayRef.current) {
        scheduleConsultTimer(() => {
          sayRef.current?.(botPlayer, "vols-envide", bubbleDurationMs);
          if (partnerIsBot) {
            cancelPendingSpontaneous(partner);
            ...
            scheduleConsultTimer(() => {
              respondWithDedup(partner, displayPhrase, sayVars, bubbleDurationMs);
            ...
            // Compañero humano: garantizem que tinga com a mínim
            // CONSULT_HUMAN_TIMEOUT_MS (8s) des del moment en què veu la
            // pregunta per a contestar (regla primordial de bloqueo de
            // turn després de preguntar). Si ja queda menys, estenem el
            // timeout perquè el bot no tire abans d'hora.
            const cur = pendingSecondWaitRef.current;
            if (cur && cur.waitKey === waitKey) {
              window.clearTimeout(cur.timer);
              const extendedId = window.setTimeout(
                () => finalize(null),
                CONSULT_HUMAN_TIMEOUT_MS,
              ) as unknown as number;
              cur.timer = extendedId;
            }
          }
        }, askDirectlyNoWait ? 0 : questionDelayMs);

      } else if (!mandatoryWaitCompleted && partnerIsBot && sayRef.current) {
        ...
        if (shouldProactivelyOrderEnvit(partnerEnvit)) {
          // El bot primer de la parella en la 1a baza sempre diu "Envida!"
          // (mai "Tira la falta!"), sense que el peu-bot li haja preguntat,
          // si té 31 o més. Amb 33 no pot callar per aleatorietat: si calla,
          // se li pot passar l'envit a la parella.
          const instruction: ChatPhraseId = "envida";
          // Iniciativa pròpia del bot company (categoria "indicacio"),
          // PERÒ es tracta d'un canto reglamentari d'envit: ha d'eixir
          // de manera directa i ràpida (sense esperar 1500 ms ni que el
          // company li pregunte). El fre de 1500 ms només aplica a
          // comentaris d'adorn / frases aleatòries, mai a un canto
          // reglamentari quan el bot té la iniciativa de joc.
          // Mantenim la clau única per a que el dispatch de play-card
          // del seu company puga cancel·lar el bocadillo si el peu-bot
          // tira carta abans que arribe a pintar-se.
          // IMPORTANT: separem el ritme de joc (dispatch d'envit) del
          // temporitzador de xat. El dispatch va per `partnerBotTimer`
          // (tracked dins `pendingSecondWaitRef`) i es neteja amb la
          // resta del flux normal; així el ritme de cartes no depèn
          // del bocadillo i no s'acumulen finalize() obsolets entre
          // bazas.
          ...
          initiativeChatTimers.start([
            {
              key: `chat-iniciativa-${partner}`,
              at: 2000,
              fn: () => {
                // Cancel·la si el company del bot (= el peu-bot) ja ha tirat
                // carta en la baza actual abans d'arribar a pintar el missatge.
                const curMatch = matchRef.current;
                const curTrick = curMatch?.round.tricks[curMatch.round.tricks.length - 1];
                if (curTrick?.cards.some((tc) => tc.player === botPlayer)) return;
                // Cancel·la si durant l'espera el company li ha enviat una
                // pregunta de chat: ha de prioritzar respondre.
                const lastQ = questionReceivedAtRef.current.get(partner) ?? 0;
                if (lastQ > scheduledAt) return;
    ...
    if (
      ...
      scheduleConsultTimer(() => {
        ...
        if (!asEspasesPlayedFirstTrick(match.round)) {
          bubbleWait = sayRef.current?.(botPlayer, "a-tu") ?? 0;
        }
        scheduleConsultTimer(() => {
    ...
    if (
      ...
      if (partnerSaid.length > 0) {
        ...
        timerRef.current = window.setTimeout(() => {
          ...
          if (action) dispatch(botPlayer, action);
        }, openerPlayDelay(decideDelayMs)) as unknown as number;
        return () => {
          if (timerRef.current) window.clearTimeout(timerRef.current);
        };
      }
        ...
        if (polarized) {
          ...
          scheduleConsultTimer(() => {
            ...
            if (partnerIsHumanQ) {
              ...
              cancelPendingSpontaneous(partnerQ);
              ...
              scheduleConsultTimer(() => {
                respondWithDedup(partnerQ, answerQ);
                scheduleConsultTimer(() => {
    ...
    if (shouldStartConsult) {
      ...
      scheduleConsultTimer(() => {
        ...
        if (partnerIsHuman) {
          ...
          scheduleConsultTimer(() => {
            respondWithDedup(partner, answer, undefined, bubbleDurationMs);
            scheduleConsultTimer(() => {
              consultAdviceRef.current.set(consultKey, advice);
              const hints = buildHints();
              const action = botDecide(match, botPlayer, advice, hints, tuningRef.current, bluffRateRef.current);
  ...
  }, [match, dispatch, options.paused, mandatorySecondWaitTick, cardFreezeTick]);
  ...
  useEffect(() => {
    ...
    for (const peuBot of [1, 2, 3] as PlayerId[]) {
      ...
      const t = window.setTimeout(() => {
        if (pendingSpontaneousRef.current[peuBot]?.timer === t) {
          delete pendingSpontaneousRef.current[peuBot];
        }
        sayRef.current?.(peuBot, phrase);
      }, PEU_SPONTANEOUS_INFO_DELAY_MS);
      pendingSpontaneousRef.current[peuBot] = { phrase, timer: t };
  ...
  useEffect(() => {
    ...
    const partnerEnvit = playerTotalEnvit(r, partner);
    if (!shouldProactivelyOrderEnvit(partnerEnvit)) return;
  ...
  // avanza en 8 s y sigue siendo legal jugar, fuerza una re-decisión y
  ...
  useEffect(() => {
    ...
    watchdogTimerRef.current = window.setTimeout(() => {
      try {
        // La espera obligatoria del segundo bot de la pareja tiene prioridad
        // absoluta: el watchdog nunca puede forzar una acción mientras siga
        // activa. Al finalizar, mandatorySecondWaitTick rearma este efecto.
        if (pendingSecondWaitRef.current?.mandatory) return;
        if (isBotPlayBlockedByPartnerConsult(turn)) return;
    ...
    options.paused,
    options.userPaused,
    mandatorySecondWaitTick,
    isBotPlayBlockedByPartnerConsult,
    dispatch,
  ]);
